...
def db_result(db_path):
    ...
    for line in f:
        ...
        for i in range(len(one_db)):
            if i == 3:
                print '{:<4} {:<16} {:<4}  '.format(i, label[i], str(datetime.utcfromtimestamp(int(one_db[i]))))
            elif i == 5:
                model = which_model(one_db[i])
                print '{:<4} {:<16} {:<4}  '.format(i, label[i], model, one_db[i].decode('utf-8').encode(code))
            elif i == 9:
                app = valid_app(one_db[i])
                print '{:<4} {:<16} {:<4}  '.format(i, label[i], app, one_db[i].decode('utf-8').encode(code))
            elif i == 10:
                province = valid_province(one_db[i])
                print '{:<4} {:<16} {:<4}  '.format(i, label[i], province, one_db[i].decode('utf-8').encode(code))
            else:
                print '{:<4} {:<16} {:<4}  '.format(i, label[i], one_db[i].decode('utf-8').encode(code))
