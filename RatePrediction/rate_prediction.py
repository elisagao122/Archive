...
def rate_predict(train, train_label, test, test_label):
    ...
    for i, C in enumerate(10. ** np.arange(2, 3)):
        ...
        clf = GradientBoostingClassifier(n_estimators=100, learning_rate=1.0, max_depth=3, random_state=0).fit(train, train_label)
        result_gbrt = clf.predict(test)
        result_gbrt_proba = clf.predict_proba(test)
        ...
        for i in range(len(result_gbrt)):
            if test_label[i] == 0 and result_gbrt[i] == 0:
                pay_to_pay += 1;
            if test_label[i] == 0 and result_gbrt[i] == 1:
                pay_to_rej += 1;
            if test_label[i] == 1 and result_gbrt[i] == 0:
                rej_to_pay += 1;
            if test_label[i] == 1 and result_gbrt[i] == 1:
                rej_to_rej += 1;
